...
##### plot a single fit (the first cell arbitrarily in the matrix) from the 2 component mixture. 
cell = cmd[[2]][[1]]
cell = MclustDR(cell)
...
# compare mixture g = 1,3,4,5,6 to G = 2 
#test bic difference 
...
p = ggplot(dsub %>% filter(mean < 15), aes(x = mean, fill = parameter)) + 
  ...
  facet_wrap(~model) + 
  ggsci::scale_fill_d3(alpha = 0.4)  + 
  ggtitle("Cells with G=4 best fit ")
...
# plot example cell with BIC minimum of G = 3 
cellsub = ls[[2]][1,1]
...
p = ggplot(mrdf , aes(x = G, y = BIC, fill = G)) +
  ...
  ylab(" BIC ")
ggsave(p, filename = paste0(figpath, "mixture_component_vs_bic.pdf"), width = 10 ,height = 5)  
