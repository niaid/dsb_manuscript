...
# protein library size 
pdf(file = paste0(figpath, "lib_size_distribution.pdf"), width = 5, height = 4)
hist(log10(Matrix::colSums(cbind(adt1,adt2))),
     breaks = 50, col = "#3e8ede",
     cex.main = 0.8,
     xlab = "log10 library size",
     main =  " protein library size distribution ")
dev.off()
...
# visualize the relationship between the per cell primary latent component noise vector and the per cell protein library zise 
# run cell intrinsig gaussian mixture model (on each batch)
library(mclust)
...
tm$barcode_check = names(cm)
mr$barcode_check = names(cm)
...
isotypes = c("MouseIgG1kappaisotype_PROT","MouseIgG2akappaisotype_PROT", "Mouse IgG2bkIsotype_PROT", "RatIgG2bkIsotype_PROT")
