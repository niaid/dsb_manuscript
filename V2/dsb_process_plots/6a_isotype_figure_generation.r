...
p = ggplot(plot_sub, aes(x = log10(nUMI_Prot), y = -1*(noise_vector))) + 
  ...
  theme(axis.title.y = element_text(size = 16)) 
p
ggsave(p, filename = paste0(figpath, "row_lineage_noise_vector_vs_libsize.pdf"), width = 7, height = 3.3)  
...
p2 = ggplot(data = df, aes(x = iso_mean, y = mean.1))  + 
  ...
  ylab(" background mean µ1  ")
p2
ggsave(p2, filename = paste0(figpath,"meanisovs_meanbackground.pdf"), width = 3.3, height = 4.3)
