...
p = ggplot(plot_sub, aes(x = log10(nUMI_Prot), y = -1*(noise_vector))) + 
  ...
  theme(axis.title.x = element_text(size = 19)) +
  theme(axis.title.y = element_text(size = 19)) 
ggsave(p, filename = paste0(figpath, "lineage_noise_vector_vs_libsize.pdf"), width = 5, height = 5)  
...
ggsave(p, filename = paste0(figpath, "celltype_noise_vector_vs_libsize.pdf"), width = 11.5, height = 5)  
...
p2 = ggplot(data = df, aes(x = iso_mean, y = mean.1))  + 
  ...
  ylab(" background mean µ1  ")
ggsave(p2, filename = paste0(figpath,"meanisovs_meanbackground.pdf"), width = 3, height = 4)

p3 = plot_grid(p1, p2, ncol = 1)
ggsave(p3, filename = paste0(figpath,"merged_background_vs_lib.pdf"), width = 2.8, height = 5)
