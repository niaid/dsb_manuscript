...
pmain = ggplot(merged, aes(x = mean_negative_droplet, y = mean_unstained_control)) + 
  ...
  ggrepel::geom_text_repel(data = merged %>% filter(prot %in% plabel), 
                           aes(label = prot), size = 2.5,force = TRUE,
...
bin_plot =  list(geom_bin2d(bins = 200, show.legend = TRUE), 
                 theme_bw(), 
                 ggpubr::stat_cor(method = "pearson"), 
                 scale_fill_viridis_c(option = "B"), 
                 theme(legend.position = c(0.8, 0.3), legend.key.size = unit(0.25, units = "cm")), 
