...
#######################
### Visualizations 
df_dsb = readRDS(here('V2/missionbio_tapestri/generated_data/missionbio tapestridf_dsb.rds'))
df_clr = readRDS(here('V2/missionbio_tapestri/generated_data/missionbio tapestridf_clr.rds'))
...
p = ggplot(dsb , aes(x = prot, y = DSB, fill = prot)) + 
  theme_bw() + 
  theme(strip.background = element_blank(), axis.text.x = element_text(size = 10)) + 
  geom_violin(scale = "area", trim = FALSE, show.legend = F, draw_quantiles = TRUE) +
  ylim(c(-7, 32)) + 
  ...
  ylab("dsb") + xlab("")+ 
  coord_flip() 
ggsave(p, filename = paste0(figpath, "vln_prots_tapestri.pdf"),width = 6.5, height = 2.8)
...
p = ggplot(clr, aes(x = prot, y = CLR, fill = prot)) + 
  ...
  ylab("CLR") + xlab("")+ 
  coord_flip() 
ggsave(p, filename = paste0(figpath, "CLR_vln_prots_tapestri.pdf"),width = 6.5, height = 2.8)
