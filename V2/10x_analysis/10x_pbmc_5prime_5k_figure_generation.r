...
prot_plot = c("CD19_TotalSeqC", "CD3_TotalSeqC", "CD14_TotalSeqC",  "CD4_TotalSeqC" , "CD56_TotalSeqC", "CD8a_TotalSeqC")
neg_cells = readRDS(file = here("V2/10x_analysis/generated_data/10X PBMC 5` 5K V2neg_prot2.rds"))
pos_cells = readRDS(file = here("V2/10x_analysis/generated_data/10X PBMC 5` 5K V2pos_prot.rds"))
df_dsb = readRDS(file = here("V2/10x_analysis/generated_data/10X PBMC 5` 5K V2dsb_merged_result.RDS"))
...
plotsub = dsb %>% filter(DSB > -3) %>% filter(DSB < 27) 
...
pheatmap::pheatmap(adt_plot, 
                   color = viridis::viridis(19, option = "B"),
                   breaks =seq(min(adt_plot), to = max(adt_plot),  length.out = 20), 
...
isotypes = rownames(adt_log)[13:15]
