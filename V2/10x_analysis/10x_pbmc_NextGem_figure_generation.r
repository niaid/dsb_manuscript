...
# labeled umap 
cu = pals::cols25(n = length(unique(dsb$clusters))) %>% unlist(use.names = FALSE)
cu
p = ggplot(dsb, aes(x = UMAP_1, y = UMAP_2)) + 
  ...
  geom_point(mapping = aes(color = clusters), size = 0.5, shape = 16, alpha = 0.2, show.legend = FALSE) + 
...
ggsave(p, filename = paste0(figpath, project_title, "19_3_mg.pdf"), width = 3, height = 3)  
...
# 
# stats10x = round(c(nprot, ncells, posmean, n_background, negmean, cor_tech_size)) 
# nmes(stats10x) = c("n_prot",	"n_cells",	"prot. Avg.",	"n_drops",	"neg prot. Avg.", "cor_tech_size")
# 
